...
import { mineCompetitorGaps } from "@/lib/gemini";
...
export async function POST(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const competitorGaps = await mineCompetitorGaps(serializeLead(existing));

    const lead = await prisma.lead.update({
      where: { id },
      data: { competitorGaps, competitorGapsGeneratedAt: new Date() },
    });
